...
impl RepoSidePanel {
    ...
    const fn default_target(self) -> RepoSidePanelTab {
        match self {
            Self::Status => RepoSidePanelTab::StatusSummary,
            Self::Files => RepoSidePanelTab::Subview(RepoSubview::Status),
            Self::Branches => RepoSidePanelTab::Subview(RepoSubview::Branches),
            Self::Commits => RepoSidePanelTab::Subview(RepoSubview::Commits),
            Self::Stash => RepoSidePanelTab::Subview(RepoSubview::Stash),
        }
    }
...
enum RepoListScrollTarget {
    MainUnstaged,
    MainStaged,
    SidePanelStatusUnstaged,
    SidePanelStatusStaged,
    SidePanelBranches,
    SidePanelRemotes,
    SidePanelTags,
    SidePanelCommits,
...
#[derive(Debug, Clone, Copy)]
struct RepoSidePanelTabDef {
    label: &'static str,
    target: RepoSidePanelTab,
}

const STATUS_PANEL_TABS: [RepoSidePanelTabDef; 1] = [RepoSidePanelTabDef {
    label: "Status",
    target: RepoSidePanelTab::StatusSummary,
}];

const FILES_PANEL_TABS: [RepoSidePanelTabDef; 3] = [
    RepoSidePanelTabDef {
        label: "Files",
        target: RepoSidePanelTab::Subview(RepoSubview::Status),
    },
    RepoSidePanelTabDef {
        label: "Worktrees",
        target: RepoSidePanelTab::Subview(RepoSubview::Worktrees),
    },
    RepoSidePanelTabDef {
        label: "Submodules",
        target: RepoSidePanelTab::Subview(RepoSubview::Submodules),
    },
];

const BRANCH_PANEL_TABS: [RepoSidePanelTabDef; 3] = [
    RepoSidePanelTabDef {
        label: "Local branches",
        target: RepoSidePanelTab::Subview(RepoSubview::Branches),
    },
    RepoSidePanelTabDef {
        label: "Remotes",
        target: RepoSidePanelTab::Subview(RepoSubview::Remotes),
    },
    RepoSidePanelTabDef {
        label: "Tags",
        target: RepoSidePanelTab::Subview(RepoSubview::Tags),
    },
];
...
const COMMIT_PANEL_TABS: [RepoSidePanelTabDef; 4] = [
    ...
    RepoSidePanelTabDef {
        label: "Reflog",
        target: RepoSidePanelTab::Subview(RepoSubview::Reflog),
    },
];

const STASH_PANEL_TABS: [RepoSidePanelTabDef; 2] = [
    RepoSidePanelTabDef {
        label: "Stash",
        target: RepoSidePanelTab::Subview(RepoSubview::Stash),
    },
    RepoSidePanelTabDef {
        label: "Command log",
        target: RepoSidePanelTab::CommandLog,
    },
];

fn repo_side_panel_tabs(panel: RepoSidePanel) -> &'static [RepoSidePanelTabDef] {
    match panel {
        RepoSidePanel::Status => &STATUS_PANEL_TABS,
        RepoSidePanel::Files => &FILES_PANEL_TABS,
        RepoSidePanel::Branches => &BRANCH_PANEL_TABS,
        RepoSidePanel::Commits => &COMMIT_PANEL_TABS,
        RepoSidePanel::Stash => &STASH_PANEL_TABS,
    }
}

#[derive(Debug, Clone, Copy)]
struct RepoSplitLayout {
    side_panels: [Rect; 5],
    main_unstaged: Rect,
    main_staged: Rect,
    main_detail: Rect,
...
impl TuiApp {
    ...
    fn handle_input(&mut self, input: InputEvent) -> ReduceResult {
        match input {
            ...
            InputEvent::MouseLeft { column, row } => {
                let actions = self.route_mouse_left(column, row);
                if actions.is_empty() {
                    ReduceResult {
                        state: self.state.clone(),
                        effects: Vec::new(),
            ...
            InputEvent::MouseWheelUp { column, row } => {
                ...
                if actions.is_empty() {
                    ...
                    let mut result = self.apply_actions(actions);
                    if local_changed && result.effects.is_empty() {
                        result
                            .effects
                            .push(super_lazygit_core::Effect::ScheduleRender);
                    }
                    result
                }
            }
            InputEvent::MouseWheelDown { column, row } => {
                let (actions, local_changed) = self.route_mouse_wheel(column, row, false);
                if actions.is_empty() {
                    ReduceResult {
                        state: self.state.clone(),
                        effects: if local_changed {
    ...
    fn apply_actions<I>(&mut self, actions: I) -> ReduceResult
    where
    ...
    fn repo_scroll_state(&self, target: RepoListScrollTarget) -> ListViewportState {
        self.repo_scroll.get(&target).copied().unwrap_or_default()
    }

    fn set_repo_scroll(&mut self, target: RepoListScrollTarget, offset: usize) {
        let state = self.repo_scroll.entry(target).or_default();
        state.offset = offset;
        state.detached = true;
    }
    ...
    fn repo_scroll_metrics(
        ...
        match target {
            RepoListScrollTarget::MainUnstaged => {
                let entries =
                    super_lazygit_core::visible_status_entries(repo_mode, PaneId::RepoUnstaged);
                let len = entries.len();
                ...
                Some((
                    ...
                    repo_status_footer_lines(PaneId::RepoUnstaged),
                ))
            }
            RepoListScrollTarget::MainStaged => {
                let entries =
                    super_lazygit_core::visible_status_entries(repo_mode, PaneId::RepoStaged);
                let len = entries.len();
                let selected_position = repo_mode
                    .staged_view
                    .selected_index
                ...
                Some((
                    ...
                    selected_position,
                    repo_status_header_lines(),
                    repo_status_footer_lines(PaneId::RepoStaged),
                ))
            }
            RepoListScrollTarget::SidePanelStatusUnstaged => {
                let entries =
                ...
                let selected_position = repo_mode
                    ...
                    .unwrap_or(0)
                    .min(len.saturating_sub(1));
                Some((len, selected_position, 1, 0))
            }
            RepoListScrollTarget::SidePanelStatusStaged => {
                let entries =
                    super_lazygit_core::visible_status_entries(repo_mode, PaneId::RepoStaged);
            ...
            RepoListScrollTarget::SidePanelCommandLog => {
                Some((self.state.status_messages.len(), 0, 1, 0))
            }
            _ => {
                ...
                match target {
                    ...
                    | RepoListScrollTarget::DetailBranches => {
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailBranches {
                                9 + usize::from(
                    ...
                    | RepoListScrollTarget::DetailRemotes => {
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailRemotes {
                                8 + usize::from(
                                    repo_filter_summary_line(
                    ...
                    RepoListScrollTarget::SidePanelTags | RepoListScrollTarget::DetailTags => {
                        let visible_indices =
                            visible_tag_indices(detail, repo_mode.tags_filter.query.as_str());
                        Some((
                            visible_indices.len(),
                            visible_position(&visible_indices, repo_mode.tags_view.selected_index),
                            if target == RepoListScrollTarget::DetailTags {
                                8 + usize::from(
                                    repo_filter_summary_line(
                    ...
                    | RepoListScrollTarget::DetailCommits => {
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailCommits {
                                9
                            } else {
                                1
                            },
                    ...
                    RepoListScrollTarget::SidePanelStash | RepoListScrollTarget::DetailStash => {
                        let visible_indices =
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailStash {
                                6 + usize::from(
                                    repo_filter_summary_line(
                    ...
                    RepoListScrollTarget::DetailStashFiles => {
                        ...
                        let selected_position = repo_mode
                            ...
                            .unwrap_or(0)
                            .min(len.saturating_sub(1));
                        Some((len, selected_position, 5, 0))
                    }
                    RepoListScrollTarget::SidePanelReflog | RepoListScrollTarget::DetailReflog => {
                        let visible_indices =
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailReflog {
                                9 + usize::from(
                                    repo_filter_summary_line(
                    ...
                    RepoListScrollTarget::SidePanelWorktrees
                    | RepoListScrollTarget::DetailWorktrees => {
                        let visible_indices = visible_worktree_indices(
                            detail,
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailWorktrees {
                                6 + usize::from(
                                    repo_filter_summary_line(
                                        repo_mode.worktree_filter.query.as_str(),
                    ...
                    RepoListScrollTarget::SidePanelSubmodules
                    | RepoListScrollTarget::DetailSubmodules => {
                        let visible_indices = visible_submodule_indices(
                            detail,
                        ...
                        Some((
                            ...
                            if target == RepoListScrollTarget::DetailSubmodules {
                                8 + usize::from(
                                    repo_filter_summary_line(
                                        repo_mode.submodules_filter.query.as_str(),
                    ...
                    | RepoListScrollTarget::SidePanelStatusUnstaged
                    | RepoListScrollTarget::SidePanelStatusStaged
                    | RepoListScrollTarget::SidePanelCommandLog => None,
                }
            }
        }
    }
    ...
    fn scroll_repo_list(
        ...
        let Some((len, selected_position, header_lines, footer_lines)) =
            self.repo_scroll_metrics(target)
        else {
            return false;
        };
        ...
        let visible_capacity = viewport_lines
            ...
            .max(1);
        let max_start = len.saturating_sub(visible_capacity);
        let current_start = detached_list_window(
            self.repo_scroll_state(target),
            selected_position,
            len,
            viewport_lines,
            header_lines,
        ...
        .0;
        let next_start = if up {
            current_start.saturating_sub(1)
        } else {
            (current_start + 1).min(max_start)
        };
        if next_start == current_start {
            return false;
        }
        self.set_repo_scroll(target, next_start);
        true
    }
    ...
    fn route_mouse_left(&self, column: u16, row: u16) -> Vec<Action> {
        ...
        match self.state.settings.screen_mode {
            ScreenMode::FullScreen => match self.fullscreen_repo_pane() {
                PaneId::RepoUnstaged | PaneId::RepoStaged => {
                    if point_in_rect(column, row, body) {
                        vec![Action::SetFocusedPane(self.fullscreen_repo_pane())]
                    } else {
                        Vec::new()
                    }
                }
                _ => {
                    let panel = self.active_repo_side_panel();
                    self.route_repo_side_panel_click(panel, body, column, row)
                }
            },
            _ => {
                let layout = self.repo_split_layout(body);
                for panel in RepoSidePanel::ALL {
                    let rect = layout.side_panels[panel.index()];
                    let actions = self.route_repo_side_panel_click(panel, rect, column, row);
                    if !actions.is_empty() {
                        return actions;
                    }
                }
        ...
        match self.state.settings.screen_mode {
            ...
            ScreenMode::FullScreen => {
                if point_in_rect(column, row, body) {
                    ...
                    return match pane {
                        PaneId::RepoUnstaged | PaneId::RepoStaged => self.route_repo_status_wheel(
                            pane,
                            usize::from(inner_rect(body).height),
                            up,
                        ),
                        _ => self.route_repo_detail_wheel(usize::from(inner_rect(body).height), up),
                    };
                }
                (Vec::new(), false)
            }
            _ => {
                ...
                if point_in_rect(column, row, split.main_unstaged) {
                    return self.route_repo_status_wheel(
                        PaneId::RepoUnstaged,
                        usize::from(inner_rect(split.main_unstaged).height),
                        up,
                    );
                }
                if point_in_rect(column, row, split.main_staged) {
                    return self.route_repo_status_wheel(
                        PaneId::RepoStaged,
                        usize::from(inner_rect(split.main_staged).height),
                        up,
                    );
                }
                if point_in_rect(column, row, split.main_detail) {
                    return self.route_repo_detail_wheel(
                        usize::from(inner_rect(split.main_detail).height),
                        up,
                    );
                }
                (Vec::new(), false)
    ...
    fn route_repo_side_panel_click(
        &self,
        panel: RepoSidePanel,
        area: Rect,
        column: u16,
        ...
        if point_in_rect(column, row, inner) && row == inner.y {
            for (target, target_rect) in repo_side_panel_tab_hitboxes(panel, selected_tab, inner) {
                if point_in_rect(column, row, target_rect) {
                    return self.repo_side_panel_action(target).into_iter().collect();
                }
            }
        }
    ...
    fn route_repo_side_panel_item_click(
        ...
        match (panel, selected_tab) {
            (RepoSidePanel::Files, RepoSidePanelTab::Subview(RepoSubview::Status)) => {
                ...
                let target = if pane == PaneId::RepoStaged {
                    RepoListScrollTarget::SidePanelStatusStaged
                } else {
                    RepoListScrollTarget::SidePanelStatusUnstaged
                };
                ...
                .0 + relative_row.saturating_sub(1);
                Some(vec![
            ...
            | (RepoSidePanel::Stash, RepoSidePanelTab::Subview(subview @ RepoSubview::Stash)) => {
                let target = match subview {
                    RepoSubview::Worktrees => RepoListScrollTarget::SidePanelWorktrees,
                    RepoSubview::Submodules => RepoListScrollTarget::SidePanelSubmodules,
                    RepoSubview::Branches => RepoListScrollTarget::SidePanelBranches,
    ...
    fn route_repo_side_panel_wheel(
        ...
        match (panel, selected_tab) {
            (RepoSidePanel::Files, RepoSidePanelTab::Subview(RepoSubview::Status)) => {
                ...
                (
                    vec![Action::SetFocusedPane(pane)],
                    self.scroll_repo_list(target, usize::from(inner_rect(area).height), up),
                )
            }
            (RepoSidePanel::Files, RepoSidePanelTab::Subview(subview @ RepoSubview::Worktrees))
            | (
            ...
            (RepoSidePanel::Stash, RepoSidePanelTab::CommandLog) => (
                Vec::new(),
                self.scroll_repo_list(
                    RepoListScrollTarget::SidePanelCommandLog,
                    usize::from(inner_rect(area).height),
                    up,
                ),
            ),
            _ => (Vec::new(), false),
    ...
    fn route_repo_status_click(
        ...
        let scroll = detached_list_window(
            self.repo_scroll_state(target),
            selected_position,
            entries.len(),
            usize::from(inner.height),
    ...
    fn route_repo_status_wheel(
        ...
        (
            vec![Action::SetFocusedPane(pane)],
            self.scroll_repo_list(target, viewport_lines, up),
        )
    }
    ...
    fn route_repo_detail_click(&self, area: Rect, column: u16, row: u16) -> Vec<Action> {
        ...
        let Some((header_lines, window_start, window_len)) =
            self.repo_detail_click_window(repo_mode, usize::from(inner.height))
        else {
            return vec![Action::SetFocusedPane(PaneId::RepoDetail)];
        };
        if relative_row < header_lines {
    ...
    fn route_repo_detail_wheel(&mut self, viewport_lines: usize, up: bool) -> (Vec<Action>, bool) {
        let Some(repo_mode) = self.state.repo_mode.as_ref() else {
            return (Vec::new(), false);
        };

        let mut actions = vec![Action::SetFocusedPane(PaneId::RepoDetail)];
        if repo_detail_uses_diff_scroll(repo_mode) {
            actions.push(if up {
            ...
            (actions, false)
        } else if let Some(target) = repo_detail_scroll_target(repo_mode) {
            (actions, self.scroll_repo_list(target, viewport_lines, up))
        } else {
            (actions, false)
        }
    }
    ...
    fn repo_detail_click_window(
        ...
        match repo_mode.active_subview {
            RepoSubview::Branches => {
                ...
                let (window_start, window_len) = detached_list_window(
                    self.repo_scroll_state(RepoListScrollTarget::DetailBranches),
                    selected_position,
                    visible_indices.len(),
                    viewport_lines,
                    header_lines,
            ...
            RepoSubview::Stash => match repo_mode.stash_subview_mode {
                StashSubviewMode::List => {
                    ...
                    let (window_start, window_len) = detached_list_window(
                        self.repo_scroll_state(RepoListScrollTarget::DetailStash),
                        selected_position,
                        visible_indices.len(),
                        viewport_lines,
                        header_lines,
                ...
                StashSubviewMode::Files => {
                    ...
                    let header_lines = 5;
                    let (window_start, window_len) = detached_list_window(
                        self.repo_scroll_state(RepoListScrollTarget::DetailStashFiles),
                        selected_position,
                        len,
                        viewport_lines,
                        header_lines,
    ...
    fn render_repo_shell(&self, area: Rect, buffer: &mut Buffer, theme: Theme) {
        ...
        match self.state.settings.screen_mode {
            ...
            ScreenMode::FullScreen => {
                self.render_repo_side_panel(
                    self.active_repo_side_panel(),
                    layout[1],
                    buffer,
                    theme,
                );
            ...
            ScreenMode::Normal | ScreenMode::HalfScreen => {
                let split = self.repo_split_layout(layout[1]);
                for panel in RepoSidePanel::ALL {
                    self.render_repo_side_panel(
                        panel,
                        split.side_panels[panel.index()],
                        buffer,
                        theme,
                    );
                }
                self.render_repo_main_cluster(split, buffer, theme);
            }
        }
    }
    ...
    fn repo_split_layout(&self, area: Rect) -> RepoSplitLayout {
        let columns = split_repo_columns(area, self.repo_side_shell_width(area));
        let side_panels = split_repo_side_panels(columns[0], self.active_repo_side_panel());
        let main = split_repo_columns(columns[1], self.repo_main_cluster_width(columns[1]));
        let left = split_repo_left_column(main[0]);
        RepoSplitLayout {
            side_panels: [
                side_panels[0],
                side_panels[1],
            ...
            main_unstaged: left[0],
            main_staged: left[1],
            main_detail: main[1],
        }
    }

    fn repo_side_shell_width(&self, area: Rect) -> u16 {
        let detail_focus = self.fullscreen_repo_pane() == PaneId::RepoDetail;
        match self.state.settings.screen_mode {
            ScreenMode::HalfScreen if detail_focus => {
                if area.width >= 140 {
                    58
    ...
    fn repo_main_cluster_width(&self, area: Rect) -> u16 {
        if area.width >= 120 {
            38
    ...
    fn active_repo_side_panel(&self) -> RepoSidePanel {
        self.state
            .repo_mode
            .as_ref()
            .map(|repo_mode| active_repo_side_panel(repo_mode.active_subview))
            .unwrap_or(RepoSidePanel::Files)
    }

    fn selected_repo_side_tab(&self, panel: RepoSidePanel) -> RepoSidePanelTab {
        let Some(repo_mode) = self.state.repo_mode.as_ref() else {
            return panel.default_target();
        };
        match panel {
            RepoSidePanel::Status => RepoSidePanelTab::StatusSummary,
            RepoSidePanel::Files => match repo_mode.active_subview {
                RepoSubview::Worktrees => RepoSidePanelTab::Subview(RepoSubview::Worktrees),
                RepoSubview::Submodules => RepoSidePanelTab::Subview(RepoSubview::Submodules),
                _ => RepoSidePanelTab::Subview(RepoSubview::Status),
            },
            RepoSidePanel::Branches => match repo_mode.active_subview {
                RepoSubview::Remotes => RepoSidePanelTab::Subview(RepoSubview::Remotes),
                RepoSubview::Tags => RepoSidePanelTab::Subview(RepoSubview::Tags),
            ...
            RepoSidePanel::Stash => {
                if self.state.pending_menu.as_ref().is_some_and(|menu| {
                    menu.operation == super_lazygit_core::MenuOperation::CommandLog
                }) && self
                    .state
                    .modal_stack
                    .last()
                    .is_some_and(|modal| matches!(modal.kind, super_lazygit_core::ModalKind::Menu))
                {
    ...
    fn repo_side_panel_action(&self, target: RepoSidePanelTab) -> Option<Action> {
        match target {
            RepoSidePanelTab::StatusSummary => Some(Action::SwitchRepoSubview(RepoSubview::Status)),
            RepoSidePanelTab::Subview(subview) => Some(Action::SwitchRepoSubview(subview)),
            RepoSidePanelTab::CommandLog => Some(Action::OpenCommandLog),
        }
    }

    fn render_repo_side_panel(
        &self,
        panel: RepoSidePanel,
        area: Rect,
        buffer: &mut Buffer,
    ...
    fn repo_side_panel_preview_lines(
        &self,
        panel: RepoSidePanel,
        ...
        let Some(repo_mode) = self.state.repo_mode.as_ref() else {
            return vec![Line::from("Repository detail is still loading.")];
        };

        match panel {
            RepoSidePanel::Status => self.repo_status_summary_lines(repo_mode, available_lines),
            RepoSidePanel::Files => self.repo_files_panel_lines(repo_mode, available_lines),
            RepoSidePanel::Branches => self.repo_branches_panel_lines(repo_mode, available_lines),
            RepoSidePanel::Commits => self.repo_commits_panel_lines(repo_mode, available_lines),
            RepoSidePanel::Stash => self.repo_stash_panel_lines(repo_mode, available_lines),
        }
    }

    fn repo_status_summary_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        ...
        let mut lines = vec![
            ...
            Line::from(format!(
                ...
                summary.map_or(0, |summary| summary.staged_count),
                summary.map_or(0, |summary| summary.unstaged_count),
                summary.map_or(0, |summary| summary.untracked_count)
            )),
            Line::from(format!(
                "Sync: +{}/-{}  Watch: {}",
                summary.map_or(0, |summary| summary.ahead_count),
                summary.map_or(0, |summary| summary.behind_count),
                watcher_health_label(&self.state.workspace.watcher_health)
            )),
            Line::from(format!(
                "Progress: {}",
    ...
    fn repo_files_panel_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        match self.selected_repo_side_tab(RepoSidePanel::Files) {
            RepoSidePanelTab::Subview(RepoSubview::Worktrees) => {
                self.compact_worktree_lines(repo_mode, available_lines)
            }
            RepoSidePanelTab::Subview(RepoSubview::Submodules) => {
                self.compact_submodule_lines(repo_mode, available_lines)
            }
            _ => self.compact_status_lines(repo_mode, available_lines),
        }
    }

    fn repo_branches_panel_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        match self.selected_repo_side_tab(RepoSidePanel::Branches) {
            RepoSidePanelTab::Subview(RepoSubview::Remotes) => {
                self.compact_remote_lines(repo_mode, available_lines)
            }
            RepoSidePanelTab::Subview(RepoSubview::Tags) => {
                self.compact_tag_lines(repo_mode, available_lines)
            }
            _ => self.compact_branch_lines(repo_mode, available_lines),
        }
    }

    fn repo_commits_panel_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        match self.selected_repo_side_tab(RepoSidePanel::Commits) {
            RepoSidePanelTab::Subview(RepoSubview::Reflog) => {
                self.compact_reflog_lines(repo_mode, available_lines)
            }
            _ => self.compact_commit_lines(repo_mode, available_lines),
        }
    }

    fn repo_stash_panel_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        match self.selected_repo_side_tab(RepoSidePanel::Stash) {
            RepoSidePanelTab::CommandLog => self.compact_command_log_lines(available_lines),
            _ => self.compact_stash_lines(repo_mode, available_lines),
        }
    }

    fn compact_status_lines(
        &self,
        repo_mode: &RepoModeState,
        available_lines: usize,
    ) -> Vec<Line<'static>> {
        ...
        let selected_index = match pane {
            ...
            _ => repo_mode.status_view.selected_index,
        };
        let mut lines = vec![Line::from(format!(
            ...
            query_label(repo_mode.status_filter.query.as_str())
        ))];
        if entries.is_empty() {
    ...
    fn compact_worktree_lines(
        ...
        let filter = repo_mode
            .subview_filter(RepoSubview::Worktrees)
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_worktree_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Worktrees: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No worktrees available."));
            return lines;
        }
        ...
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelWorktrees),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_submodule_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_submodule_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Submodules: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No submodules available."));
            return lines;
        }
        let selected_index = repo_mode
            .submodules_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelSubmodules),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
            ...
            .take(window_len)
        {
            ...
            let marker = if index == selected_index { ">" } else { " " };
            let state = if submodule.conflicted {
                "conflicted"
    ...
    fn compact_branch_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_branch_indices(detail, filter, repo_mode.branch_sort_mode);
        let mut lines = vec![Line::from(format!(
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No local branches available."));
            return lines;
        }
        let selected_index = repo_mode
            .branches_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelBranches),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_remote_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_remote_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Remotes: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelRemotes),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_tag_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_tag_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Tags: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No tags available."));
            return lines;
        }
        let selected_index = repo_mode
            .tags_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelTags),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_commit_lines(
        ...
        let mut lines = vec![Line::from(format!(
            ...
            visible_indices.len(),
            query_label(repo_mode.commits_filter.query.as_str())
        ))];
        if visible_indices.is_empty() {
            lines.push(Line::from("No commits available."));
            return lines;
        }
        let selected_index = repo_mode
            .commits_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelCommits),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_reflog_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_reflog_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Reflog: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No reflog entries available."));
            return lines;
        }
        let selected_index = repo_mode
            .reflog_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelReflog),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
            ...
            .take(window_len)
        {
            ...
            lines.push(Line::from(format!(
                "{marker} {} {} {}",
                entry.selector, entry.short_oid, entry.summary
            )));
        }
        lines.truncate(available_lines);
        lines
    ...
    fn compact_stash_lines(
        ...
        let filter = repo_mode
            ...
            .map(|filter| filter.query.as_str())
            .unwrap_or("");
        let visible_indices = visible_stash_indices(detail, filter);
        let mut lines = vec![Line::from(format!(
            "Stashes: {}  filter:{}",
            visible_indices.len(),
            query_label(filter)
        ...
        if visible_indices.is_empty() {
            lines.push(Line::from("No stashes available."));
            return lines;
        }
        let selected_index = repo_mode
            .stash_view
        ...
        let selected_position = visible_position(&visible_indices, Some(selected_index));
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelStash),
            selected_position,
            visible_indices.len(),
            available_lines,
            1,
    ...
    fn compact_command_log_lines(&self, available_lines: usize) -> Vec<Line<'static>> {
        let mut lines = vec![Line::from(format!(
            "Messages: {}",
            self.state.status_messages.len()
        ))];
        if self.state.status_messages.is_empty() {
            lines.push(Line::from("No command log entries available."));
            return lines;
        }
        let messages = self.state.status_messages.iter().rev().collect::<Vec<_>>();
        let (window_start, window_len) = detached_list_window(
            self.repo_scroll_state(RepoListScrollTarget::SidePanelCommandLog),
            0,
            messages.len(),
            available_lines,
            1,
...
fn active_repo_side_panel(subview: RepoSubview) -> RepoSidePanel {
    match subview {
        RepoSubview::Status | RepoSubview::Worktrees | RepoSubview::Submodules => {
            RepoSidePanel::Files
        }
        RepoSubview::Branches
        | RepoSubview::Remotes
        | RepoSubview::RemoteBranches
        | RepoSubview::Tags => RepoSidePanel::Branches,
        RepoSubview::Commits | RepoSubview::Compare | RepoSubview::Rebase | RepoSubview::Reflog => {
            RepoSidePanel::Commits
        }
        RepoSubview::Stash => RepoSidePanel::Stash,
    }
}
...
fn repo_side_panel_tab_hitboxes(
    ...
    for (index, tab) in repo_side_panel_tabs(panel).iter().enumerate() {
        ...
        if width > 0 && cursor < right {
            let clamped_width = width.min(right.saturating_sub(cursor));
            hitboxes.push((tab.target, Rect::new(cursor, inner.y, clamped_width, 1)));
...
fn detached_list_window(
    ...
    let (centered_start, centered_len) = centered_list_window(
        selected_position,
        len,
        viewport_lines,
        header_lines,
    ...
    if len == 0 || !scroll_state.detached {
        return (centered_start, centered_len);
    }

    let visible_capacity = viewport_lines
        .saturating_sub(header_lines.saturating_add(footer_lines))
        .max(1);
    let window_start = scroll_state
        .offset
        .min(len.saturating_sub(visible_capacity));
    let window_end = (window_start + visible_capacity).min(len);
    (window_start, window_end.saturating_sub(window_start))
}
...
fn repo_detail_scroll_target(repo_mode: &RepoModeState) -> Option<RepoListScrollTarget> {
    match repo_mode.active_subview {
        RepoSubview::Branches => Some(RepoListScrollTarget::DetailBranches),
        RepoSubview::Remotes => Some(RepoListScrollTarget::DetailRemotes),
        RepoSubview::RemoteBranches => Some(RepoListScrollTarget::DetailRemoteBranches),
        RepoSubview::Tags => Some(RepoListScrollTarget::DetailTags),
        RepoSubview::Commits => Some(
            if repo_mode.commit_subview_mode == CommitSubviewMode::Files
                && repo_mode.commit_files_mode == CommitFilesMode::List
            {
                RepoListScrollTarget::DetailCommitFiles
        ...
        RepoSubview::Stash => match repo_mode.stash_subview_mode {
            StashSubviewMode::List => Some(RepoListScrollTarget::DetailStash),
            StashSubviewMode::Files => Some(RepoListScrollTarget::DetailStashFiles),
        },
        RepoSubview::Reflog => Some(RepoListScrollTarget::DetailReflog),
        RepoSubview::Worktrees => Some(RepoListScrollTarget::DetailWorktrees),
        RepoSubview::Submodules => Some(RepoListScrollTarget::DetailSubmodules),
        RepoSubview::Status | RepoSubview::Compare | RepoSubview::Rebase => None,
    }
}
...
fn repo_stash_lines(
    ...
    is_focused: bool,
    viewport_lines: usize,
    list_scroll_state: ListViewportState,
    file_scroll_state: ListViewportState,
    theme: Theme,
) -> Vec<Line<'static>> {
    let Some(detail) = detail else {
        return vec![
...
fn repo_commit_lines(
    ...
    if repo_mode.commit_subview_mode == CommitSubviewMode::Files {
        return match repo_mode.commit_files_mode {
            CommitFilesMode::List => repo_commit_file_list_lines(
                repo_mode,
                selected,
                viewport_lines,
                commit_file_scroll_state,
                theme,
            ),
...
fn repo_subview_tabs(active: RepoSubview) -> Vec<Span<'static>> {
    let active_panel = active_repo_side_panel(active);
    let all = [
        (RepoSidePanel::Status, "[1] Status"),
        (RepoSidePanel::Files, "[2] Files / Worktrees / Submodules"),
        (
            RepoSidePanel::Branches,
            "[3] Local branches / Remotes / Tags",
        ),
        (RepoSidePanel::Commits, "[4] Commits / Reflog"),
        (RepoSidePanel::Stash, "[5] Stash / Command log"),
    ];

    let mut spans = Vec::with_capacity(all.len() * 2);
    for (index, (panel, label)) in all.iter().enumerate() {
        let rendered = if *panel == active_panel {
            format!("{label}*")
        } else {
            label.to_string()
        };
...
mod tests {
    ...
    #[test]
    fn route_repository_mouse_click_switches_grouped_repo_tabs() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let state = AppState {
            mode: AppMode::Repository,
            focused_pane: PaneId::RepoDetail,
        ...
        let area = Rect::new(
            0,
            0,
            app.viewport().width.max(1),
            app.viewport().height.max(1),
        );
        let body = Layout::default()
            .direction(Direction::Vertical)
            .constraints([Constraint::Length(3), Constraint::Min(3)])
            .split(area)[1];
        let split = app.repo_split_layout(body);
        let panel = RepoSidePanel::Files;
        let inner = inner_rect(split.side_panels[panel.index()]);
        let selected = app.selected_repo_side_tab(panel);
        let (_, target) = repo_side_panel_tab_hitboxes(panel, selected, inner)
            .into_iter()
            .find(|(tab, _)| *tab == RepoSidePanelTab::Subview(RepoSubview::Worktrees))
            .expect("worktrees tab hitbox");

        let result = app.dispatch(Event::Input(InputEvent::MouseLeft {
            column: target.x.saturating_add(1),
            row: target.y,
        }));
    ...
    fn route_repository_mouse_click_focuses_main_repo_panes() {
        ...
        let unstaged = app.dispatch(Event::Input(InputEvent::MouseLeft {
            column: split
                .main_unstaged
                .x
                .saturating_add(split.main_unstaged.width / 2),
            row: split
                .main_unstaged
                .y
                .saturating_add(split.main_unstaged.height / 2),
        }));
        assert_eq!(unstaged.state.focused_pane, PaneId::RepoUnstaged);

        let staged = app.dispatch(Event::Input(InputEvent::MouseLeft {
            column: split
                .main_staged
                .x
                .saturating_add(split.main_staged.width / 2),
            row: split
                .main_staged
                .y
                .saturating_add(split.main_staged.height / 2),
        }));
        assert_eq!(staged.state.focused_pane, PaneId::RepoStaged);
    }

    #[test]
    fn route_repository_mouse_wheel_scrolls_status_viewport_without_changing_selection() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let mut detail = sample_repo_detail();
        detail.file_tree.extend((0..8).map(|index| FileStatus {
            path: PathBuf::from(format!("src/generated_{index}.rs")),
            kind: FileStatusKind::Modified,
            staged_kind: None,
            unstaged_kind: Some(FileStatusKind::Modified),
            ..FileStatus::default()
        ...
        let body = Layout::default()
            ...
            .split(area)[1];
        let split = app.repo_split_layout(body);
        let inner = inner_rect(split.main_unstaged);

        let result = app.dispatch(Event::Input(InputEvent::MouseWheelDown {
            column: inner.x.saturating_add(1),
            row: inner.y.saturating_add(repo_status_header_lines() as u16),
        }));
        ...
        assert_eq!(
            app.repo_scroll_state(RepoListScrollTarget::MainUnstaged)
                .offset,
            1
        );
        assert!(
    ...
    #[test]
    fn route_repository_mouse_wheel_scrolls_detail_viewport_without_changing_selection() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let mut detail = sample_repo_detail();
        detail.branches.extend((0..8).map(|index| BranchItem {
        ...
        let mut app = TuiApp::new(state, AppConfig::default());
        let (actions, local_changed) = app.route_repo_detail_wheel(10, false);
        ...
        assert_eq!(actions, vec![Action::SetFocusedPane(PaneId::RepoDetail)]);
        assert!(local_changed);
        assert_eq!(
            app.repo_scroll_state(RepoListScrollTarget::DetailBranches)
                .offset,
            1
        );
        assert!(
    ...
    #[test]
    fn route_repository_mouse_click_uses_scrolled_main_status_window() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let mut detail = sample_repo_detail();
        detail.file_tree.extend((0..8).map(|index| FileStatus {
            path: PathBuf::from(format!("src/generated_{index}.rs")),
        ...
        let mut app = TuiApp::new(state, AppConfig::default());
        app.set_repo_scroll(RepoListScrollTarget::MainUnstaged, 2);
    ...
    #[test]
    fn route_repository_mouse_click_uses_scrolled_detail_window() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let mut detail = sample_repo_detail();
        detail.branches.extend((0..8).map(|index| BranchItem {
            name: format!("feature-{index}"),
        ...
        let mut app = TuiApp::new(state, AppConfig::default());
        app.set_repo_scroll(RepoListScrollTarget::DetailBranches, 2);

        let area = Rect::new(0, 0, 80, 16);
        let inner = inner_rect(area);

        let actions =
            app.route_repo_detail_click(area, inner.x.saturating_add(1), inner.y.saturating_add(9));

        assert_eq!(
            actions,
            vec![
                Action::SetFocusedPane(PaneId::RepoDetail),
                Action::SelectRepoDetailItem { index: 2 },
            ]
        );
    }

    #[test]
    fn route_repository_mouse_click_uses_scrolled_side_panel_window() {
        let repo_id = RepoId::new("/tmp/repo-1");
        let mut detail = sample_repo_detail();
        detail.branches.extend((0..8).map(|index| BranchItem {
            name: format!("feature-{index}"),
        ...
        let mut app = TuiApp::new(state, AppConfig::default());
        app.resize(120, 18);
        app.set_repo_scroll(RepoListScrollTarget::SidePanelBranches, 2);
        ...
        let body = Layout::default()
            ...
            .split(area)[1];
        let split = app.repo_split_layout(body);
        let panel_rect = split.side_panels[RepoSidePanel::Branches.index()];
        let inner = inner_rect(panel_rect);

        let actions = app
            .route_repo_side_panel_item_click(
                RepoSidePanel::Branches,
            ...
            .expect("side panel actions");

        assert_eq!(
            actions,
            vec![
                Action::SwitchRepoSubview(RepoSubview::Branches),
                Action::SetFocusedPane(PaneId::RepoDetail),
                Action::SelectRepoDetailItem { index: 2 },
            ]
        );
    ...
    fn render_repo_shell_shows_tag_details() {
        ...
        assert!(help.contains("Ctrl+O copy tag"));
