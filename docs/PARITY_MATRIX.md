...
Current open clone-parity beads: `supergit-15v`, `supergit-2ri`.
...
| Repo-shell grouped layout and mouse interaction | Lazygit renders five side windows (`status`, `files`, `branches`, `commits`, `stash`) with grouped tabs, click-to-focus and tab-click switching, plus enlarged/full focused-side-window behavior. | Gap | `supergit-2ri` | None yet; add grouped-shell render, mouse-routing, and side-window screen-mode regressions as part of `supergit-2ri` | Local TUI still renders a `Repository shell` header with one flat tab row and a three-pane `unstaged/staged/detail` body, so the visible shell is not yet lazygit-clone parity. |
...
| Status/log graph and release-reference controls | Status-origin log graph, tag browsing, tag lifecycle, and related release-reference controls behave like lazygit instead of acting as placeholders. | Partial | `supergit-3o1.2`, `supergit-3o1.10`, `supergit-3o1.15`, `supergit-15v` | `route_repository_status_patch_menu_key`, `route_repository_status_tree_and_shell_keys`, `repo_mode_tags_detail_routes_filter_navigation_and_actions`, `e2e_keyboard_harness_runs_tag_filter_create_push_delete_cycle` | Tag and release-reference handling are shipped, but clone-grade Unicode commit graph rendering is still open in `supergit-15v`. |
